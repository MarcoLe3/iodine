...
Limit tool usage and output: read only the specific files or line ranges needed, prefer open_file for focused excerpts, and use search_files with a narrow query/path. Do not read an entire large file unless it is necessary. When inspecting a large file, inspect it in focused sections.
When modifying an existing file, use edit_file — supply the exact block to replace and the new content. Only use write_file when creating a brand-new file. If edit_file returns an error because old_string was not found or matched multiple times, read the file again and retry with a more unique surrounding context. Never use placeholder comments like "// rest of file unchanged" or "// ..." in any file write.
