...
export function useToolNarration(provider: Provider) {
  const queueRef = useRef<Array<() => Promise<string>>>([]);
  ...
  const drain = useCallback(async () => {
    ...
    while (queueRef.current.length && generation === generationRef.current) {
      const fetchAudio = queueRef.current.shift()!;
      try {
  ...
  const narrate = useCallback((name: string, input: Record<string, unknown>) => {
    const path = Object.values(input).find(value => typeof value === 'string' && (value.includes('/') || value.includes('\\'))) as string | undefined;
    ...
    queueRef.current.push(async () => {
      const response = await fetch(`${API_BASE}/api/tts/speak`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: phrase, provider: provider.id }) });
  ...
  return { narrate, stop, drain, queueRef, audioRef, hadNarrationsRef, onEmptyRef, resetTurn };
}
