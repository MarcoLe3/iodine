...
function MessageBubble({ msg, isLast, sendApproval, onSuggestion }: { msg: UIMessage; isLast: boolean; sendApproval: (id: string, approved: boolean) => void; onSuggestion: (text: string) => void }) {
  ...
  return <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, color: 'var(--color-text-secondary)', marginBottom: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><img src="/logo.png" alt="Iodine" style={{ width: 14, height: 14, objectFit: 'contain' }} />Assistant</span><span style={{ fontWeight: 400, fontSize: 10 }}>{formatTime(msg.timestamp)}</span></div><div>{msg.blocks.map((block, i) => { if (block.type === 'text') { const showCursor = isStreaming && isLast && i === msg.blocks.length - 1; return <div key={i} className="md-body"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ code({ className, children, ...props }) { const isBlock = className?.startsWith('language-'); return isBlock ? <pre className="md-pre"><code className={className} {...props}>{children}</code></pre> : <code className="md-code-inline" {...props}>{children}</code>; } }}>{block.content}</ReactMarkdown>{showCursor && <span style={{ animation: 'blink 1s step-end infinite', opacity: 1 }}>▌</span>}</div>; } if (block.type === 'thought') return <ThoughtBlock key={i} block={block} />; if (block.type === 'command-approval') return <CommandApprovalBlock key={block.id} block={block} onApprove={() => sendApproval(block.id, true)} onReject={() => sendApproval(block.id, false)} />; return <ToolBlock key={block.id} block={block} />; })}{hasFileTool && !isStreaming && <button onClick={() => onSuggestion('Open file for me')} style={{ background: 'var(--color-bg-subtle)', border: '1px solid var(--color-border)', borderRadius: 999, color: 'var(--color-text-secondary)', cursor: 'pointer', fontSize: 11, padding: '5px 10px', marginTop: 4 }}>Open file for me</button>}{isStreaming && msg.blocks.length === 0 && <span style={{ fontSize: 13, color: 'var(--color-text-secondary)', animation: 'blink 1s step-end infinite' }}>▌</span>}</div></div>;
}
